...
const MedicationDetailsScreen: React.FC<Props> = ({route, navigation}) => {
  ...
  const isActive: boolean = medication.isActive ?? raw.isActive ?? raw.is_active ?? true;
  const createdAt: Date | undefined = medication.createdAt ?? (raw.created_at ? new Date(raw.created_at) : undefined);
  const updatedAt: Date | undefined = medication.updatedAt ?? (raw.updated_at ? new Date(raw.updated_at) : undefined);
  ...
  const handleEditSchedule = () => {
    // Note: MedicationSchedule expects medication without reminderTimes
    // The schedule will be regenerated
    const { reminderTimes: _, ...medicationWithoutTimes } = medication;
    navigation.navigate('MedicationSchedule', {
      medication: {
        ...medicationWithoutTimes,
        drugName: name,
        dosage,
        frequency,
        strength,
