...
const MedicationScheduleScreen: React.FC<Props> = ({route, navigation}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        try {
          ...
          try {
            const medsFromBackend = await import('@services/BackendService').then(m => m.fetchMedications());
            const dbMed: any = medsFromBackend.find((m) => String(m.id) === String(medication.id));
            if (dbMed) {
              const mappedMed: Medication = {
                id: String(dbMed.id),
                user_key: dbMed.user_key,
                drugName: dbMed.drug_name || dbMed.drugName || '',
                dosage: dbMed.qty_text || dbMed.dosage || '',
                frequency: dbMed.frequency_text || dbMed.frequency || '',
                strength: dbMed.strength || '',
                instructions: dbMed.instruction || dbMed.instructions || '',
                reminderTimes: adjustedTimes,
                startDate: new Date(),
                medication_key: dbMed.medication_key,
              };
        ...
        // Always navigate to Home after edit
        navigation.reset({
          index: 0,
          routes: [{ name: 'Home' }],
        });
        ...
        // Always navigate to LinkRFID after save
    ...
    if (saveError) {
      Alert.alert('Warning', `Medication was saved, but there was a problem: ${saveError.message || saveError}`);
    }
  };
