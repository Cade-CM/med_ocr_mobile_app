...
const MedicationConfirmationScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const backendMeds = user_key
        ? await StorageService.getMedicationsFromBackend(user_key)
        : [];
  ...
  const confirmMedication = async (
    ...
    try {
      ...
      const user_key = await AsyncStorage.getItem('user_key');
      if (user_key && medPayload.medication_key) {
        // For event logging, label_name is not sent, so no change needed here
        const eventPayload = {
          user_key,
