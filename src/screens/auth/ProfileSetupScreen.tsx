...
import { requireUserKey, AuthRequiredError } from '@services/AuthService';
...
const ProfileSetupScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Get user_key from session - will throw if not authenticated
      console.log('ProfileSetup: Getting user_key from session...');
      const user_key = await requireUserKey();
      console.log('ProfileSetup: Got user_key:', user_key);
      
      // Update profile - user_key is optional now since service derives from session
      console.log('ProfileSetup: Updating profile...');
      ...
      if (error instanceof AuthRequiredError) {
        console.log('ProfileSetup: AuthRequiredError - redirecting to Login');
        if (Platform.OS === 'web') {
      ...
      if (Platform.OS === 'web') {
        window.alert('Failed to save profile: ' + (error?.message || 'Unknown error'));
      } else {
