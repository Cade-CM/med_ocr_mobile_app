...
const DashboardScreen: React.FC = () => {
  ...
  const safeMissedDoses = useMemo(() => {
    if (Array.isArray(stats?.missedDoses)) {
      return stats.missedDoses.length;
    }
    if (typeof stats?.missedDoses === 'number') {
      return stats.missedDoses;
    }
    return 0;
  }, [stats?.missedDoses]);

  const adherenceColor = useMemo(() => {
    const percentage = stats.adherencePercentage;
